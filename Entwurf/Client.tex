...
\section{Client} 
	...
	\subsection{ServerConnection}
 	...
 	ServerConnection benutzt zusätzlich von android.os die Klasse AsyncTask um die Serveranfrage im Hintergrund auszuführen.
...
\begin{itemize}
	\item public void httpRequestExecuted(JSONObject answer)
		\begin{description}
		\item The code in this method is executed after the server send his response back to the 			        AsyncTask. This method is responsible for handling the response.
	 \item @param answer The JSONObject to the JSON-string send back from the servlet.
		\end{description}
	\end{itemize}
	
	\subsubsection{public class GetRequest extends AsyncTask}
This Class is responsible for sending a get-request to the server and calling the httpRequestExecuted method on the AsyncResponse object. GetRequest extends android.os.AsyncTask.
	...
	\item public GetRequest(String JSON, AsyncResponse response)
		\begin{description}
		\item Constructor which expects a JSON-string and an AsyncResponse of the server request.
	 \item @param JSON The String which should be send to the server
	 \item @param response The AsyncResponse that should be executed after receiving the answer from the server.
		\end{description}
	\end{itemize}
...
\begin{itemize}
	\item public HTTPConnection(String nameOfServlet)
	 ...
	 \item @param nameOfServlet The name of the servlet, the service wants to communicate with.
		...
		\item public void sendGetRequest(String JSON,AsyncResponse response)
		\begin{description}
\item Method that handles a get request.
	 ...
	 \item @param response The AsyncResponse that should be executed after receiving the answer from the server.
		...
		\item public void sendPostRequest(String JSON,AsyncResponse response)
		\begin{description}
