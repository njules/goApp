package pse.goApp.client.controler;

import pse.goApp.client.model.*;

/**
 * This Service provides methods to handle a users reaction towards an event
 */
public class ParticipateService {

	/**
	 * The user is added to the event as a participant this includes starting a Timer for the Notification Broadcast
	 * @param event
	 * @param user
	 */
	private void accept(Event event, User user) {
		// TODO - implement ParticipateService.accept
		throw new UnsupportedOperationException();
	}

	/**
	 * the connection between the user and the event is deleted
	 * @param event
	 * @param user
	 */
	private void reject(Event event, User user) {
		// TODO - implement ParticipateService.reject
		throw new UnsupportedOperationException();
	}

}