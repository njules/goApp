...
\usepackage{hyperref}
\usepackage[acronym,xindy,toc,nonumberlist]{glossaries} % nomain, if you define glossaries in a file, and you use \include{INP-00-glossary}
\makeglossaries
...
\begin{document}
	...
	\begin{itemize}
		...
		\item Ein User kann \gls{Mitglied} mehrerer Gruppen sein und kann Name, Mitglieder, Gründer und Termine einsehen.
	...
	\begin{itemize}
		\item[FA160] Jeder \gls{Teilnehmer} kann den Gruppenstandort ab 15 Minuten in einer Karte darstellen lassen.
		\item[FA165W] Vom Gruppenmittelpunkt entfernte Teilgruppen  erhalten einen eigenen Standort. 
		\item[FA170W] \gls{Teilnehmer} können mittels eines Buttons Signale an die anderen Teilnehmer senden:
		\begin{itemize}
	...
	\begin{itemize}
		\item[T30] Ein Benutzer erstellt eine neue Gruppe hierzu muss:
			...
			\item der entsprechende Benutzer als \gls{Gruppengrnder} eingetragen werden
			\end{itemize}
		...
		\item[T50] Einer existierende Gruppe beitreten:
			...
			\item bei einer Annahme ist der Benutzer dann \gls{Mitglied} der Gruppe
	...
	\begin{itemize}
		\item[T70] Ein beliebiges Gruppenmitglied erstellt einen neuen Termin hierzu muss:
			...
			\item der entsprechende Benutzer als Terminadministrator und damit auch als \gls{Teilnehmer} eingetragen werden
		...
		\item[WT80] Terminverwaltung durch den Terminadministrator:
			...
			\end{itemize}
		\item[T90] Ein \gls{Mitglied} lässt sich einen Termin anzeigen und erhält folgende Informationen:
			\begin{itemize}
			\item Terminname, Treffpunkt und Zeitpunkt
			\item wer den Termin gegründet hat
			\item die Liste aller \gls{Teilnehmer}
			\item nur bei eigener Teilnahme und bei Terminstart: den Gruppenmittelpunkt
			\end{itemize}
		\item[T100] Ein \gls{Mitglied} sagt bei einem Termin zu daraufhin wird:
			\begin{itemize}
			\item das Mitglied zum \gls{Teilnehmer}
			\item in die Teilnehmerliste aufgenommen
			\item vor Beginn des Termins benachrichtigt
			\end{itemize}
		\item[T110] Ein \gls{Mitglied} sagt bei einem Termin ab woraufhin der Termin nicht mehr angezeigt wird. 
	\end{itemize}

	\subsection{Standort}
	\begin{itemize}
		\item[T120] Der \gls{Server} lokalisiert alle \gls{Teilnehmer} eines Termins eine viertel Stunde vor Terminbeginn und berechnet den gemittelten Standort. 
		\item[T130] Der \gls{Server} löscht nach Ende des Termins alle Informationen über die Standorte der \gls{Teilnehmer}. 
	 \end{itemize}	
