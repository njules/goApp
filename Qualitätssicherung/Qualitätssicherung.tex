...
\subparagraph{public void testSetStatusWOutJSON()}
Die Methode setStatus wird aufgerufen, jedoch fehlt die UserID als Parameter. Das Servlet soll den Fehlercode READ\_JSON zurückgeben.
\subparagraph{public void testSetStatusDBUpdateFalse()}
Beim aktualisieren des Status in de Datenbank tritt ein Fehler auf. Vom Servlet wird der DB\_ERROR Fehler erwartet.
\subparagraph{public void testSetStatusDBDeleteFalse()}
Wird der Status auf rejected gesetzt, soll der Benutzer aus der Datenbank gelöscht werden. Hierbei tritt ein Fehler auf und das Servlet gibt einen DB\_ERROR zurück.
\subparagraph{public void testSetStatusDBTrue()}
Es wird getestet, ob das entfernen des Benutzers aus der Datenbank erfolgreich verläuft, wenn er seinen Status auf rejected setzen will.
\subparagraph{public void testMethodNotExisting()}
Dieser Test überprüft, ob das Servlet einen METH\_ERROR zurückgibt, wenn versucht wird eine Methode aufzurufen, die in diesem Serlvet nicht existiert.
\subparagraph{public void testEmptyJSON()}
In diesem Test wird eine Anfrage mit einem Leeren JSON Objekt gesendet. Das Servlet soll einen READ\_JSON Fehler zurückgeben.
...
\begin{center}
	\makebox [0pt]{	\includegraphics[height=0.79\textheight]{Codeabdeckung.jpg}}
\end{center}
\newpage
\section{Client}
